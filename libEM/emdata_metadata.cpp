...
MArray2D EMData::get_2dview(unsigned long x0, unsigned long y0) const
{
	const int ndims = 2;
	if (get_ndim() != ndims) {
		throw ImageDimensionException("2D only");
...
MArray3D EMData::get_3dview(unsigned long x0, unsigned long y0, unsigned long z0) const
{
	const int ndims = 3;
	boost::array<std::size_t,ndims> dims = {{nx, ny, nz}};
	MArray3D marray(get_data(), dims, boost::fortran_storage_order());
...
MCArray2D EMData::get_2dcview(unsigned long x0, unsigned long y0) const
{
	const int ndims = 2;
	if (get_ndim() != ndims) {
		throw ImageDimensionException("2D only");
...
MCArray3D EMData::get_3dcview(unsigned long x0, unsigned long y0, unsigned long z0) const
{
	const int ndims = 3;
	boost::array<std::size_t,ndims> dims = {{nx/2, ny, nz}};
	std::complex<float>* cdata = reinterpret_cast<std::complex<float>*>(get_data());
